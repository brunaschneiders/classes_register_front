...
import { People, EditTwoTone, Delete } from '@material-ui/icons';
...
export default () => {
  ...
  const [
    ...
    setShowConfirmationDeleteGrowdevClassModal,
  ] = useState(false);
  ...
  async function handleShowConfirmationDeleteGrowdevClassModal(growdevClass) {
    setShowConfirmationDeleteGrowdevClassModal(true);
    setSelectedGrowdevClassUid(growdevClass.uid);
  }

  async function handleDeleteGrowdevClass() {
    setShowConfirmationDeleteGrowdevClassModal(false);
    setLoading(true);
    try {
      const response = await api.delete(`/classes/${selectedGrowdevClassUid}`, {
        headers: { Authorization: `Bearer ${userData.token}` },
      });
      setLoading(false);
      if (response?.data?.message) {
    ...
    <Container>
      <Row style={{ padding: '2%' }}>
        ...
        <Button
          variant="contained"
          color="secondary"
          onClick={() => handleShowCreateGrowdevClassModal()}
        >
                ...
                <CardActions style={{ justifyContent: 'center' }}>
                  ...
                  <Tooltip title="Atualizar aula">
                    <IconButton
                      style={{ padding: '0px 10px' }}
                      onClick={() =>
                        handleShowUpdateGrowdevClassModal(growdevClass)
                      }
                    >
                      <EditTwoTone color="secondary" />
                    </IconButton>
                  ...
                  <Tooltip title="Cancelar aula">
                    <IconButton
                      style={{ padding: '0px 10px' }}
                      onClick={() =>
                        handleShowConfirmationDeleteGrowdevClassModal(
                          growdevClass
                        )
                      }
                    >
